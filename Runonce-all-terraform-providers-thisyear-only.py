...
class TerraformRegistryStats:
    ...
    def get_provider_downloads(self, provider: Dict) -> Optional[Dict]:
        """
        Fetch download statistics for a specific provider for the current year
        """
        try:
            ...
            # Fetch download statistics for the current year
            url = f"{self.base_url}/providers/{provider_id}/downloads?year={current_year}"
            response = requests.get(url)
            response.raise_for_status()
            ...
            return {
                ...
                'downloads': download_data.get('downloads', 0),
            ...
            logging.error(f"Error fetching downloads for provider {provider.get('id')}: {str(e)}")
            return None
    ...
    def save_report(self, report_data: List[Dict]):
        ...
        output_dir = "g:/My Drive/Dharani project/tf-registry-data"
